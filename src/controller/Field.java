...
public class Field extends Canvas {
    ...
    static Field canvas = new Field(canvasWidth, canvasHeight);
    static char lastKeyTyped = 'd';
    ...
    static void ifLost() {
        int[][] coordinates = noteThePositionOfTheSnake();
        for (int i = 1; i < snake.size(); i++) {
            if (snake.get(0).getX() == snake.get(i).getX() && snake.get(0).getY() == snake.get(i).getY()) {
                try {
                    ...
                    snakePositionUpdate(coordinates);
                    break;
    ...
    private static int[][] noteThePositionOfTheSnake() {
        int[][] coordinates = new int[snake.size()][2];
        int index = 0;
        for (int i = 0; i < snake.size(); i++) {
            coordinates[index][0] = snake.get(i).getX();
            coordinates[index][1] = snake.get(i).getY();
            index++;
        }
        return coordinates;
    }

    private static void snakePositionUpdate(int[][] coordinates) {
        int index = 0;
        for (int i = 0; i < snake.size(); i++) {
            snake.get(i).setX(coordinates[index][0]);
            snake.get(i).setY(coordinates[index][1]);
            index++;
        }
    }
}
